...
public class Eos4 {
    ...
    public Eos4(int round, Date gameDate) {
        this.round = round > 360 ? round % 360 : round;
    ...
    // 스코어 입력
    public void updateScore(Eos4Dto.Score score) {
        if (score.isCancel()) {
            this.pb = "";
            this.ball = "";
            this.oddeven = "";
        ...
        switch (gameCode) {
            ...
            case "pb_even_ou":
                return "ODD".equals(pb_oddeven) ? new ZoneScore(0, 0, GameResult.NONE)
                        : ZoneHelper.zoneResult(pb_overunder);
            default:
                throw new GameZoneException("EOS4 파워볼 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }
    ...
    private Zone getPowerZone(String gameCode, String type, String teamHome, String teamAway, int idxHome, int idxAway) {
        ...
        zone.setMenuCode(MenuCode.EOS4);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 EOS 파워볼 4분", this.round));
        ...
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getEos4().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getEos4().getOdds()[idxAway]);
        zone.setGameDate(this.gameDate);
        return zone;
    }

    public boolean isChangeResult(Eos4Dto.Score score) {
