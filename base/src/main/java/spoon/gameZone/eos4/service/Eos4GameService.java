...
public class Eos4GameService {
    ...
    private final static QEos4 q = QEos4.eos4;
    ...
    public AjaxResult betting(final ZoneDto.Bet betting) {
        ...
        Eos4 eos4 = eos4Repository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = eos4.getZone(betting.getGameCode());
        double odds = eos4.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("EOS 4분 파워볼 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("EOS 4분 파워볼 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (eos4.getGameDate().getTime() - config.getBetTime() * 1000L < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS4)).and(qi.groupId.eq(eos4.getSdate())).and(qi.cancel.isFalse()));
        ...
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS4)).and(qi.groupId.eq(eos4.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
        ...
        bet.setMenuCode(MenuCode.EOS4);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(eos4.getGameDate());
        bet.setEndDate(eos4.getGameDate());
    ...
    @Transactional
    public void rollbackPayment(Eos4 eos4) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS4).and(qi.groupId.eq(eos4.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    public void closingBetting(Eos4 eos4) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS4).and(qi.groupId.eq(eos4.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = eos4.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.EOS4, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Eos4 eos4 = eos4Repository.findOne(q.sdate.eq(sdate));
            eos4.setAmount(amount);
