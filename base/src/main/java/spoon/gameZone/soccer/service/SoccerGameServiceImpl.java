...
@AllArgsConstructor
@Service
public class SoccerGameServiceImpl implements SoccerGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        Soccer soccer = soccerRepository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = soccer.getZone(betting.getGameCode());
        double odds = soccer.getOdds(betting.getBetZone());
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("가상축구 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("가상축구 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (soccer.getGameDate().getTime() - config.getBetTime() * 1000 < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.SOCCER)).and(qi.groupId.eq(soccer.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("가상축구는 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.SOCCER)).and(qi.groupId.eq(soccer.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("가상축구는 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.SOCCER);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(soccer.getGameDate());
        bet.setEndDate(soccer.getGameDate());
    ...
    @Transactional
    @Override
    public void rollbackPayment(Soccer soccer) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.SOCCER).and(qi.groupId.eq(soccer.getSdate())).and(qi.cancel.isFalse()));
    ...
    @Transactional
    @Override
    public void closingBetting(Soccer soccer) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.SOCCER).and(qi.groupId.eq(soccer.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = soccer.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            long[] amount = {0, 0, 0, 0, 0, 0, 0};
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.SOCCER, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Soccer soccer = soccerRepository.findOne(q.sdate.eq(sdate));
            soccer.setAmount(amount);
