...
@AllArgsConstructor
@Service
public class CrownSutdaGameServiceImpl implements CrownSutdaGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        CrownSutda crownSutda = crownSutdaRepository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = crownSutda.getZone(betting.getGameCode());
        double odds = crownSutda.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("섰다 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("섰다 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (crownSutda.getGameDate().getTime() - config.getBetTime() * 1000 < System.currentTimeMillis() - 60000) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.CROWN_SUTDA)).and(qi.groupId.eq(crownSutda.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("섰다는 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.CROWN_SUTDA)).and(qi.groupId.eq(crownSutda.getSdate())).and(qi.betTeam.eq(betting.getBetTeam())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("섰다는 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.CROWN_SUTDA);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(crownSutda.getGameDate());
        bet.setEndDate(crownSutda.getGameDate());
    ...
    @Transactional
    @Override
    public void rollbackPayment(CrownSutda crownSutda) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.CROWN_SUTDA).and(qi.groupId.eq(crownSutda.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    @Override
    public void closingBetting(CrownSutda crownSutda) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.CROWN_SUTDA).and(qi.groupId.eq(crownSutda.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = crownSutda.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            long[] amount = {0, 0, 0};
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.CROWN_SUTDA, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            CrownSutda crownSutda = crownSutdaRepository.findOne(q.sdate.eq(sdate));
            crownSutda.setAmount(amount);
