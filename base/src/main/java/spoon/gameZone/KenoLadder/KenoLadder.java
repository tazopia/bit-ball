...
@Data
@Entity
@Table(name = "ZONE_KENO_LADDER", indexes = {
        @Index(name = "IDX_sdate", columnList = "sdate")
})
public class KenoLadder {
    ...
    public KenoLadder(int round, Date gameDate) {
        this.round = round;
    ...
    // 스코어 입력
    public void updateScore(KenoLadderDto.Score score) {
        if (score.isCancel()) {
            this.oddeven = "";
            this.start = "";
            this.line = "";
    ...
    public ZoneScore getGameResult(String gameCode) {
        switch (gameCode) {
            ...
            default:
                throw new GameZoneException("키노사다리 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }
    ...
    private Zone getLadderZone(String gameCode, String teamHome, String teamAway, int idxHome, int idxAway) {
        ...
        zone.setMenuCode(MenuCode.KENO_LADDER);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 키노사다리", this.round));
        zone.setTeamHome(String.format("%03d회차 키노사다리 [%s]", this.round, teamHome));
        zone.setTeamAway(String.format("%03d회차 키노사다리 [%s]", this.round, teamAway));
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getKenoLadder().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getKenoLadder().getOdds()[idxAway]);
        zone.setGameDate(this.gameDate);
        return zone;
    }

    public boolean isChangeResult(KenoLadderDto.Score score) {
