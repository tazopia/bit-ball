...
public class Eos5 {
    ...
    public Eos5(int round, Date gameDate) {
    ...
    // 스코어 입력
    public void updateScore(Eos5Dto.Score score) {
        if (score.isCancel()) {
            this.pb = "";
            this.ball = "";
            this.oddeven = "";
        ...
        switch (gameCode) {
            ...
            case "pb_even_ou":
                return "ODD".equals(pb_oddeven) ? new ZoneScore(0, 0, GameResult.NONE)
                        : ZoneHelper.zoneResult(pb_overunder);
            default:
                throw new GameZoneException("EOS5 파워볼 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }
    ...
    private Zone getPowerZone(String gameCode, String type, String teamHome, String teamAway, int idxHome, int idxAway) {
        ...
        zone.setMenuCode(MenuCode.EOS5);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 EOS 파워볼 5분", this.round));
        ...
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getEos5().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getEos5().getOdds()[idxAway]);
        zone.setGameDate(this.gameDate);
        return zone;
    }

    public boolean isChangeResult(Eos5Dto.Score score) {
