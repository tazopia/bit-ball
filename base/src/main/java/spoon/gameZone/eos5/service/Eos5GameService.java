...
public class Eos5GameService {
    ...
    private final static QEos5 q = QEos5.eos5;
    ...
    public AjaxResult betting(final ZoneDto.Bet betting) {
        ...
        Eos5 eos5 = eos5Repository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = eos5.getZone(betting.getGameCode());
        double odds = eos5.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("EOS 5분 파워볼 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("EOS 5분 파워볼 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (eos5.getGameDate().getTime() - config.getBetTime() * 1000L < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS5)).and(qi.groupId.eq(eos5.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("EOS 5분 파워볼은 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS5)).and(qi.groupId.eq(eos5.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("EOS 5분 파워볼은 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.EOS5);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(eos5.getGameDate());
        bet.setEndDate(eos5.getGameDate());
    ...
    @Transactional
    public void rollbackPayment(Eos5 eos5) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS5).and(qi.groupId.eq(eos5.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    public void closingBetting(Eos5 eos5) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS5).and(qi.groupId.eq(eos5.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = eos5.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.EOS5, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Eos5 eos5 = eos5Repository.findOne(q.sdate.eq(sdate));
            eos5.setAmount(amount);
