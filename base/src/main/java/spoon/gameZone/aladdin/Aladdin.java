...
@Data
@Entity
@Table(name = "ZONE_ALADDIN", indexes = {
...
public class Aladdin {
    ...
    public Aladdin(int round, Date gameDate) {
        this.round = round;
    ...
    // 스코어 입력
    public void updateScore(AladdinDto.Score score) {
        if (score.isCancel()) {
            this.oddeven = "";
            this.start = "";
            this.line = "";
    ...
    public Zone getZone(String gameCode) {
        switch (gameCode) {
            case "oddeven":
                return getAladdinZone("oddeven", "홀", "짝", 0, 1);
            case "start":
                return getAladdinZone("start", "좌", "우", 2, 3);
            case "line":
                return getAladdinZone("line", "3줄", "4줄", 4, 5);
            case "line3Start":
                return getAladdinZone("line3Start", "3줄/좌", "3줄/우", 6, 7);
            case "line4Start":
                return getAladdinZone("line4Start", "4줄/좌", "4줄/우", 8, 9);
            default:
                throw new GameZoneException("알라딘 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }

    private Zone getAladdinZone(String gameCode, String teamHome, String teamAway, int idxHome, int idxAway) {
        Zone zone = new Zone();
        zone.setId(this.getId());
        zone.setSdate(this.getSdate());
        zone.setMenuCode(MenuCode.ALADDIN);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 알라딘", this.round));
        zone.setTeamHome(String.format("%03d회차 알라딘 [%s]", this.round, teamHome));
        zone.setTeamAway(String.format("%03d회차 알라딘 [%s]", this.round, teamAway));
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getAladdin().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getAladdin().getOdds()[idxAway]);
        zone.setGameDate(this.gameDate);
        return zone;
    }

    public boolean isChangeResult(AladdinDto.Score score) {
