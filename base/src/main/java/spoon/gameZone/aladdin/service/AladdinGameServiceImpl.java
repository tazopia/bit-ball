...
@AllArgsConstructor
@Service
public class AladdinGameServiceImpl implements AladdinGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        Aladdin aladdin = aladdinRepository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = aladdin.getZone(betting.getGameCode());
        double odds = aladdin.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("알라딘 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("알라딘 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (aladdin.getGameDate().getTime() - config.getBetTime() * 1000 < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.ALADDIN)).and(qi.groupId.eq(aladdin.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("알라딘은 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.ALADDIN)).and(qi.groupId.eq(aladdin.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("알라딘은 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.ALADDIN);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(aladdin.getGameDate());
        bet.setEndDate(aladdin.getGameDate());
        bet.setExpMoney((long) (bet.getBetOdds() * betting.getBetMoney()));
        bet.setIp(WebUtils.ip());
        bet.setBalance(member.isBalanceAladdin());
    ...
    @Transactional
    @Override
    public void rollbackPayment(Aladdin aladdin) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.ALADDIN).and(qi.groupId.eq(aladdin.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    @Override
    public void closingBetting(Aladdin aladdin) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.ALADDIN).and(qi.groupId.eq(aladdin.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = aladdin.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.ALADDIN, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Aladdin aladdin = aladdinRepository.findOne(q.sdate.eq(sdate));
            aladdin.setAmount(amount);
