...
@AllArgsConstructor
@Service
public class SnailGameServiceImpl implements SnailGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        Snail snail = snailRepository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = snail.getZone(betting.getGameCode());
        double odds = snail.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("달팽이 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("달팽이 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (snail.getGameDate().getTime() - config.getBetTime() * 1000 < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.SNAIL)).and(qi.groupId.eq(snail.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("네임드 달팽이는 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.SNAIL)).and(qi.groupId.eq(snail.getSdate())).and(qi.betTeam.eq(betting.getBetTeam())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("네임드 달팽이는 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.SNAIL);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(snail.getGameDate());
        bet.setEndDate(snail.getGameDate());
    ...
    @Transactional
    @Override
    public void rollbackPayment(Snail snail) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.SNAIL).and(qi.groupId.eq(snail.getSdate())).and(qi.cancel.isFalse()));
    ...
    @Transactional
    @Override
    public void closingBetting(Snail snail) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.SNAIL).and(qi.groupId.eq(snail.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = snail.getGameResult();
    ...
    protected void addAmount(String sdate) {
        try {
            long[] amount = {0, 0, 0};
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.SNAIL, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Snail snail = snailRepository.findOne(q.sdate.eq(sdate));
            snail.setAmount(amount);
