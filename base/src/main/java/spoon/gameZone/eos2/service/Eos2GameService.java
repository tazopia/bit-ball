...
public class Eos2GameService {
    ...
    private final static QEos2 q = QEos2.eos2;
    ...
    public AjaxResult betting(final ZoneDto.Bet betting) {
        ...
        Eos2 eos2 = eos2Repository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = eos2.getZone(betting.getGameCode());
        double odds = eos2.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("EOS 2분 파워볼 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("EOS 2분 파워볼 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (eos2.getGameDate().getTime() - config.getBetTime() * 1000L < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS2)).and(qi.groupId.eq(eos2.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("EOS 1분 파워볼은 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS2)).and(qi.groupId.eq(eos2.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("EOS 1분 파워볼은 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.EOS2);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(eos2.getGameDate());
        bet.setEndDate(eos2.getGameDate());
    ...
    @Transactional
    public void rollbackPayment(Eos2 eos2) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS2).and(qi.groupId.eq(eos2.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    public void closingBetting(Eos2 eos2) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS2).and(qi.groupId.eq(eos2.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = eos2.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.EOS2, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Eos2 eos2 = eos2Repository.findOne(q.sdate.eq(sdate));
            eos2.setAmount(amount);
