...
@Data
@Entity
@Table(name = "ZONE_BITCOIN5", indexes = {
        @Index(name = "IDX_sdate", columnList = "sdate")
})
public class Bitcoin5 {
    ...
    private Bitcoin5(LocalDateTime gameDate) {
    ...
    public static Bitcoin5 of(LocalDateTime gameDate) {
        return new Bitcoin5(gameDate);
    }
    ...
    public boolean isChangeResult(Bitcoin5Dto.Score score) {
        return score.getHigh() != this.high || score.getLow() != this.low || !score.getBs().equals(this.bs);
    }

    public void updateScore(Bitcoin5Dto.Score score) {
        if (score.isCancel()) {
            this.closing = true;
            this.cancel = true;
            return;
    ...
    public void updateScore(Bitcoin5Json score) {
        this.low = score.getLow();
        this.high = score.getHigh();
        this.open = score.getOpen();
        this.close = score.getClose();
    ...
    private Zone getBitcoin3Zone(String gameCode, String type, String teamHome, String teamAway, int idxHome, int idxAway) {
        ...
        zone.setMenuCode(MenuCode.BITCOIN5);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 BTC 5분", this.round));
        ...
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getBitcoin5().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getBitcoin5().getOdds()[idxAway]);
