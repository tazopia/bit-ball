...
@AllArgsConstructor
@Service
public class CrownBaccaratGameServiceImpl implements CrownBaccaratGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        CrownBaccarat crownBaccarat = crownBaccaratRepository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = crownBaccarat.getZone(betting.getGameCode());
        double odds = crownBaccarat.getOdds()[betting.getBetZone()];
        ...
        // 베팅시간 체크
        if (crownBaccarat.getGameDate().getTime() - config.getBetTime() * 1000 < System.currentTimeMillis() - 60000) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.CROWN_BACCARAT)).and(qi.groupId.eq(crownBaccarat.getSdate())).and(qi.cancel.isFalse()));
        ...
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.CROWN_BACCARAT)).and(qi.groupId.eq(crownBaccarat.getSdate())).and(qi.betTeam.eq(betting.getBetTeam())).and(qi.cancel.isFalse()));
            if (count > 0) {
        ...
        bet.setMenuCode(MenuCode.CROWN_BACCARAT);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(crownBaccarat.getGameDate());
        bet.setEndDate(crownBaccarat.getGameDate());
        ...
        if (success) {
            ...
            if (bet.getRole() == Role.USER) {
                addAmount(crownBaccarat.getSdate());
            }
        } else {
    ...
    @Transactional
    @Override
    public void rollbackPayment(CrownBaccarat crownBaccarat) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.CROWN_BACCARAT).and(qi.groupId.eq(crownBaccarat.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    @Override
    public void closingBetting(CrownBaccarat crownBaccarat) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.CROWN_BACCARAT).and(qi.groupId.eq(crownBaccarat.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = crownBaccarat.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            long[] amount = {0, 0, 0};
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.CROWN_BACCARAT, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            CrownBaccarat crownBaccarat = crownBaccaratRepository.findOne(q.sdate.eq(sdate));
            crownBaccarat.setAmount(amount);
