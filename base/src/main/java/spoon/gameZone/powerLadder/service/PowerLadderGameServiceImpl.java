...
public class PowerLadderGameServiceImpl implements PowerLadderGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.POWER_LADDER)).and(qi.groupId.eq(powerLadder.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.betTeam.eq(betting.getBetTeam())).and(qi.cancel.isFalse()));
            if (count > 0) {
