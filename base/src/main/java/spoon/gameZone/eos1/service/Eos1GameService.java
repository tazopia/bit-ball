...
public class Eos1GameService {
    ...
    private final static QEos1 q = QEos1.eos1;
    ...
    public AjaxResult betting(final ZoneDto.Bet betting) {
        ...
        Eos1 eos1 = eos1Repository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = eos1.getZone(betting.getGameCode());
        double odds = eos1.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("EOS 1분 파워볼 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("EOS 1분 파워볼 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (eos1.getGameDate().getTime() - config.getBetTime() * 1000L < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS1)).and(qi.groupId.eq(eos1.getSdate())).and(qi.cancel.isFalse()));
        ...
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.EOS1)).and(qi.groupId.eq(eos1.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
        ...
        bet.setMenuCode(MenuCode.EOS1);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(eos1.getGameDate());
        bet.setEndDate(eos1.getGameDate());
    ...
    @Transactional
    public void rollbackPayment(Eos1 eos1) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS1).and(qi.groupId.eq(eos1.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    public void closingBetting(Eos1 eos1) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.EOS1).and(qi.groupId.eq(eos1.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = eos1.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.EOS1, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Eos1 eos1 = eos1Repository.findOne(q.sdate.eq(sdate));
            eos1.setAmount(amount);
