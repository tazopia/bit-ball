...
public class Eos1 {
    ...
    public Eos1(int round, Date gameDate) {
        this.round = round > 1440 ? round % 1440 : round;
    ...
    // 스코어 입력
    public void updateScore(Eos1Dto.Score score) {
        if (score.isCancel()) {
            this.pb = "";
            this.ball = "";
            this.oddeven = "";
        ...
        switch (gameCode) {
            ...
            case "pb_even_ou":
                return "ODD".equals(pb_oddeven) ? new ZoneScore(0, 0, GameResult.NONE)
                        : ZoneHelper.zoneResult(pb_overunder);
            default:
                throw new GameZoneException("EOS1 파워볼 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }
    ...
    private Zone getPowerZone(String gameCode, String type, String teamHome, String teamAway, int idxHome, int idxAway) {
        ...
        zone.setMenuCode(MenuCode.EOS1);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 EOS 파워볼 1분", this.round));
        ...
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getEos1().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getEos1().getOdds()[idxAway]);
        zone.setGameDate(this.gameDate);
        return zone;
    }

    public boolean isChangeResult(Eos1Dto.Score score) {
