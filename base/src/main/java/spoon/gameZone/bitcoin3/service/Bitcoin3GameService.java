...
@Service
public class Bitcoin3GameService {
    ...
    private static QBitcoin3 q = QBitcoin3.bitcoin3;

    @Transactional
    public void closingBetting(Bitcoin3 bitcoin3) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.BITCOIN3).and(qi.groupId.eq(bitcoin3.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            Bet bet = item.getBet();
            if (bet.isPayment()) continue;

            ZoneScore zoneScore = bitcoin3.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.BITCOIN3, sdate);
            for (BetDto.ZoneAmount bet : list) {
                amount[bet.getBetZone()] = bet.getAmount();
            }
            Bitcoin3 entity = bitcoin3Repository.findOne(q.sdate.eq(sdate));
    ...
    @Transactional
    public void rollbackPayment(Bitcoin3 entity) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.BITCOIN3).and(qi.groupId.eq(entity.getSdate())).and(qi.cancel.isFalse()));
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        Bitcoin3 entity = bitcoin3Repository.findOne(q.sdate.eq(betting.getSdate()));
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("비트코인 3분차트 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("비트코인 3분차트 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (entity.getGameDate().getTime() - (3 * 60 * 1000) - config.getBetTime() * 1000 < System.currentTimeMillis()) {
        ...
        // 중복베팅 체크
        QBetItem qi = QBetItem.betItem;
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.BITCOIN3)).and(qi.groupId.eq(entity.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("비트코인 3분 차트는 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.BITCOIN3)).and(qi.groupId.eq(entity.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("비트코인 3분 차트는 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.BITCOIN3);
        bet.setBetCount(1);
