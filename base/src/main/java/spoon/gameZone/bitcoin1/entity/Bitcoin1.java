...
@Data
@Entity
@Table(name = "ZONE_BITCOIN1", indexes = {
...
public class Bitcoin1 {
    ...
    private Bitcoin1(LocalDateTime gameDate) {
    ...
    public static Bitcoin1 of(LocalDateTime gameDate) {
        return new Bitcoin1(gameDate);
    }
    ...
    public boolean isChangeResult(Bitcoin1Dto.Score score) {
        return score.getHigh() != this.high || score.getLow() != this.low || !score.getBs().equals(this.bs);
    }

    public void updateScore(Bitcoin1Dto.Score score) {
        if (score.isCancel()) {
            this.closing = true;
            this.cancel = true;
            return;
    ...
    public void updateScore(Bitcoin1Json score) {
        this.low = score.getLow();
        this.high = score.getHigh();
        this.open = score.getOpen();
        this.close = score.getClose();
    ...
    public Zone getZone(String gameCode) {
        switch (gameCode) {
            case "hi_oe":
                return getBitcoin1Zone("hi_oe", "고가", "홀", "짝", 0, 1);
            case "hi_ou":
                return getBitcoin1Zone("hi_ou", "고가", "오버", "언더", 2, 3);
            case "lo_oe":
                return getBitcoin1Zone("lo_oe", "저가", "홀", "짝", 4, 5);
            case "lo_ou":
                return getBitcoin1Zone("lo_ou", "저가", "오버", "언더", 6, 7);
            case "hi_odd_ou":
                return getBitcoin1Zone("hi_odd_ou", "고가", "홀/오버", "홀/언더", 8, 9);
            case "hi_even_ou":
                return getBitcoin1Zone("hi_even_ou", "고가", "짝/오버", "짝/언더", 10, 11);
            case "lo_odd_ou":
                return getBitcoin1Zone("lo_odd_ou", "저가", "홀/오버", "홀/언더", 12, 13);
            case "lo_even_ou":
                return getBitcoin1Zone("lo_even_ou", "저가", "짝/오버", "짝/언더", 14, 15);
            default:
                throw new GameZoneException("비트코인 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }

    private Zone getBitcoin1Zone(String gameCode, String type, String teamHome, String teamAway, int idxHome, int idxAway) {
        Zone zone = new Zone();
        zone.setId(this.getId());
        zone.setSdate(this.getSdate());
        zone.setMenuCode(MenuCode.BITCOIN1);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 BTC 1분", this.round));
        ...
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getBitcoin1().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getBitcoin1().getOdds()[idxAway]);
