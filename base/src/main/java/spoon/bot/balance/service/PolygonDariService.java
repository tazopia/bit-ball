...
@AllArgsConstructor
@Service
public class PolygonDariService {
    ...
    public void balance() {
        ...
        double rate = Config.getGameConfig().getBalanceDariRate();
        if (rate == 0 || !Config.getGameConfig().isBalanceDari()) return;

        ZoneMaker zoneMaker = ZoneConfig.getDari().getZoneMaker();
        ...
        Iterable<BetItem> items = betGameService.getBalanceBet(MenuCode.DARI, sdate);
        ...
        if (canbet || canbet2) {
            ...
            String url = Config.getSysConfig().getZone().getBalanceUrl() + "/bet/daridari?auth=" + Config.getGameConfig().getBalancePolygon() +
            ...
            log.info("[다리다리] 단 : " + canbet + ", 투 : " + canbet2 + " / bo - " + bo + ", be - " + be + ", bl - " + bl + ", br - " + br + ", b3 - " + b3 + ", b4 - " + b4 + ", otl3 - " + otl3 + ", otl4 - " + otl4 + ", otr3 - " + otr3 + ", otr4 - " + otr4);
